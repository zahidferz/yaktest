      ...
      try {
        ...
        await this.$apollo.mutate({
          ...
          update: (store, { data }) => {
            ...
            const newLogo = data.userUpdate.image + `?${new Date().getTime()}`
            oldData.myAccount.image = newLogo
            store.writeQuery({
              ...query,
              data: oldData,
            })
...
<template>
  <div v-if="$mq === 'mo'" class="flex-grow flex flex-col items-center p-4">
    <p class="mb-8 text-center"
      >Esta fotografía aparecerá en tu perfil, podrás cambiarla cuando quieras.
    </p>
    <div>
    ...
    <div class="flex-grow flex flex-col mt-8 items-center justify-end">
      <GxButton :loading="loading" @click="onSave">Guardar</GxButton>
      <GxTextLink
  ...
  <div v-else class="flex-grow flex flex-col items-center">
    <GxDesktopHeader
      ...
      header="Ajusta tu fotografía"
      :icon-click-handler="goBack"
    />

    <!-- <header class="w-full  flex justify-start items-center mt-4">
      <i class="material-icons-round iconSize mr-4">keyboard_backspace</i>
      <h4 class="gx-h4 font-bold">Ajusta tu fotografía</h4>
    </header> -->
    <p class="text-left mt-4 self-start mb-8"
      >Esta fotografía aparecerá en tu perfil, podrás cambiarla cuando quieras.
    </p>
